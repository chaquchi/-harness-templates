...
- _（记录已验证但可能不充分的点，如未覆盖的边界条件）_
- _（记录数据库/缓存/消息队列等外部依赖的风险）_
...
- `./init.sh`
- 编译：`mvn -q -DskipTests compile` / `npm run build` / `pip install -e .`
- 测试：`mvn test` / `npm test` / `pytest`
- 启动：`mvn spring-boot:run` / `npm run dev` / `uvicorn main:app`
