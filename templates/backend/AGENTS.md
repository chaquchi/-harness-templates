...
本仓库使用 Harness 工程规范管理后端项目的 AI Coding 工作流。
...
- 代码注释要求中文，简洁、通俗易懂。方法注释说明关键作用和边界条件，类注释说明职责，关键逻辑加行内注释。
- 不要自行提交代码，提交由用户决定。
- **后端规范**：
  - Controller 层只做参数校验和路由转发，不写业务逻辑。
  - Service 层承载全部业务逻辑，方法必须是可测试的纯逻辑。
  - 所有外部调用（数据库、缓存、HTTP、MQ）必须有对应的异常处理和超时配置。
  - API 返回结构统一使用项目约定的 Response/Result 封装类。
...
- 目标行为已经实现，API 接口符合 RESTful 规范。
- 该功能要求的验证已经实际执行（编译通过、单元测试通过、集成测试通过）。
- 验证结果和证据已记录在 `feature_list.json` 的 `evidence` 字段中。
- 当前改动没有破坏 `./init.sh` 的标准启动路径。
- 新增的 API 已覆盖正常输入、异常输入、边界值三种情况的测试。
- 数据库变更（如有）已记录迁移脚本或 SQL 文件。
...
4. 清理临时调试痕迹（`System.out.println`、`console.log`、注释掉的试验代码、临时 SQL）。
