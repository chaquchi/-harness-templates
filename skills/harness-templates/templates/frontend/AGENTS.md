...
本仓库使用 Harness 工程规范管理前端项目的 AI Coding 工作流。
...
- 代码注释要求中文，简洁、通俗易懂。函数/组件注释说明关键作用，Hook/Composable 注释说明副作用和依赖。
- 不要自行提交代码，提交由用户决定。
- **前端规范**：组件使用 PascalCase 命名，目录使用 kebab-case。样式优先使用项目已有的设计系统变量，不硬编码颜色/间距。
...
- `init.sh`：统一初始化、依赖安装、lint、测试和构建验证入口。
...
- 目标行为已经实现，UI 交互符合需求。
- 该功能要求的验证已经实际执行（lint 通过、单元测试通过、构建成功）。
- 验证结果和证据已记录在 `feature_list.json` 的 `evidence` 字段中。
- 当前改动没有破坏 `./init.sh` 的标准启动路径。
- 若涉及 UI 组件，已在主流浏览器（Chrome/Firefox/Edge）中验证无渲染异常。
- 若涉及状态管理，已覆盖 loading / empty / error / success 四种状态。
...
4. 清理临时调试痕迹（`console.log`、注释掉的试验代码、无用组件/样式）。
