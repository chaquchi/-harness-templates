...
本仓库使用 Harness 工程规范管理桌面应用项目的 AI Coding 工作流。
...
- 代码注释要求中文，简洁、通俗易懂。方法注释说明关键作用，类注释说明职责，关键逻辑加行内注释。
- 不要自行提交代码，提交由用户决定。
- **桌面应用规范**：
  - Main Process 只做窗口管理和系统调用，不写业务逻辑。
  - Renderer Process 遵循前端规范，组件 PascalCase，目录 kebab-case。
  - Preload Script 只通过 contextBridge 暴露最小必要 API，遵循最小权限原则。
  - 所有 IPC 通信必须走 preload 桥接，严禁在 renderer 中直接使用 Node.js API。
  - 涉及文件系统、网络请求的操作必须有错误处理和用户反馈。
  - 打包配置（electron-builder / forge）必须验证跨平台兼容性。
...
- `init.sh`：统一初始化、依赖安装、测试、构建和打包验证入口。
...
- 目标行为已经实现，UI 交互符合需求。
- 该功能要求的验证已经实际执行（lint 通过、单元测试通过、构建成功、打包成功）。
- 验证结果和证据已记录在 `feature_list.json` 的 `evidence` 字段中。
- 当前改动没有破坏 `./init.sh` 的标准启动路径。
- 若涉及 IPC 通信，preload 暴露的 API 已做安全审查。
- 桌面应用已在目标平台（Windows/macOS/Linux）上手动 smoke 验证。
...
4. 清理临时调试痕迹（`console.log`、注释掉的试验代码、临时文件）。
